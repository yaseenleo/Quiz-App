...
export default class cssquiz extends React.Component {
         ...
         var key =["q1","q2","q3","q4"];
            ...
            for(var i=0 ; i<4 ;i++)
            {
                for(var j=0 ; j<4 ; j++)
                {
                    ...
                    if(key[i]===cssques[j][0])
                {
                    if(answer[i]===cssques[j][6])
                    {
                        count++;
                    }
                }
    ...
    var cssques2 = cssques;
    cssques2=shuffle(cssques2);
  
    console.log(cssques2);
    
        const questions = cssques2.map((qs)=>
        <div>
            <p>{qs[1]}</p>
        ...
        return(
            ...
            <div><h4 style={{"marginleft":"50px"}}>css QUIZ</h4>
